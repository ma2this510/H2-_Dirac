...
contains
   subroutine int_s11one(b_i_xi, b_i_eta, b_j_xi, b_j_eta, n_remove, knot_xi, knot_eta, Z1, Z2, m, C, R, jz2, epsilon, result)
      ...
      ! TEST 
      print *, "diff test"
      print *, "size diff", size(diff, 1), size(diff, 2)
      do i1 = 1, size(diff, 1) 
         call write_lists(diff(i1, :), 6, 25, 10)
      end do 
      print *, "diff test end"
      ! print *, "min_min test"
      ! print *, "size min_min", size(val_min_min, 1), size(val_min_min, 2)
      ! do i1 = 1, size(val_min_min, 1) 
      !    call write_lists(val_min_min(i1, :), 6, 25, 10)
      ! end do 
      ! print *, "min_min test end"
      ! END TEST

   end subroutine int_s11one

   subroutine int_s11one_no_dbl(b_i_xi, b_i_eta, b_j_xi, b_j_eta, n_remove, knot_xi, knot_eta, Z1, Z2, m, C, R, jz2, epsilon, result)
      !> @brief This subroutine calculates the S11 integral for the H2+ molecule.
      !> @param b_i_xi : real(:, :) : the B-spline coefficients for the xi direction
      !> @param b_i_eta : real(:, :) : the B-spline coefficients for the eta direction
      !> @param b_j_xi : real(:, :) : the B-spline coefficients for the xi direction
      ...
      integer :: i1, i2, j1, j2, j3, j4, alpha, beta, gamma, delta
               ...
               do j2 = 1, size(b_i_eta, 2) ! Loop over the order of eta-i
                  do j3 = 1, size(b_j_xi, 2) ! Loop over the order of xi-j
                     do j4 = 1, size(b_j_eta, 2) ! Loop over the order of eta-j
                        ...
                        val_min_min(i1, i2) = val_min_min(i1, i2) + b_i_xi(i1, j1)*b_i_eta(i2,j2)*b_j_xi(i1, j3)*b_j_eta(i2,j4)*2*mppi()*(R**3)*(knot_xi(i1)**(alpha+gamma+1)) * (knot_eta(i2)**(beta+delta+1)) *(-(knot_eta(i2)**2)/((alpha+gamma+1)*(beta+delta+3))+(knot_xi(i1)**2)/((alpha+gamma+3)*(beta+delta+1)))
                        val_max_max(i1, i2) = val_max_max(i1, i2) + b_i_xi(i1, j1)*b_i_eta(i2,j2)*b_j_xi(i1, j3)*b_j_eta(i2,j4)*2*mppi()*(R**3)*((knot_xi(i1+1))**(alpha+gamma+1))* ((knot_eta(i2+1))**(beta+delta+1)) *(-(knot_eta(i2+1)**2)/((alpha+gamma+1)*(beta+delta+3))+(knot_xi(i1+1)**2)/((alpha+gamma+3)*(beta+delta+1)))
                        val_min_max(i1, i2) = val_min_max(i1, i2) + b_i_xi(i1, j1)*b_i_eta(i2,j2)*b_j_xi(i1, j3)*b_j_eta(i2,j4)*2*mppi()*(R**3)*((knot_xi(i1))**(alpha+gamma+1))* ((knot_eta(i2+1))**(beta+delta+1)) *(-(knot_eta(i2+1)**2)/((alpha+gamma+1)*(beta+delta+3))+(knot_xi(i1)**2)/((alpha+gamma+3)*(beta+delta+1)))
                        val_max_min(i1, i2) = val_max_min(i1, i2) + b_i_xi(i1, j1)*b_i_eta(i2,j2)*b_j_xi(i1, j3)*b_j_eta(i2,j4)*2*mppi()*(R**3)*((knot_xi(i1+1))**(alpha+gamma+1))* ((knot_eta(i2))**(beta+delta+1)) *(-(knot_eta(i2)**2)/((alpha+gamma+1)*(beta+delta+3))+(knot_xi(i1+1)**2)/((alpha+gamma+3)*(beta+delta+1)))
                     end do
                  end do
               end do
            end do
   ...
   end subroutine int_s11one_no_dbl
   ...
   subroutine test_routine(d, n, n_remove, Z1, Z2, m, C, R, ximax, ximin, jz2, epsilon, eta_slp, save_step)
      ...
      print *, "Test S11one integral with cuts..."
      ! Test the S11one integral
      call int_s11one(bspline_xi(1, :, :), bspline_eta(1, :, :), &
                            bspline_xi(1, :, :), bspline_eta(1, :, :), &
                            n_remove, knotxi_eps, knoteta_eps, Z1, Z2, m, C, R, jz2, epsilon, tmp_result)

      call mpwrite(6, 30, 10, tmp_result)
      call int_s11one_no_dbl(bspline_xi(1, :, :), bspline_eta(1, :, :), &
                            bspline_xi(1, :, :), bspline_eta(1, :, :), &
                            n_remove, knotxi_eps, knoteta_eps, Z1, Z2, m, C, R, jz2, epsilon, tmp_result)
