...
private &
  ...
  mp_polygamma, mp_polygamma_be, mp_polylog_inim, mp_polylog_negm, &
...
interface polylog_ini
  module procedure mp_polylog_inim
end interface

interface polylog_neg
  module procedure mp_polylog_negm
end interface
...
1     format ('*** MP_PI: Pi must be precomputed to precision',i9,' words'/ &
  ...
  subroutine mp_polylog_inim (nn, arr, iprec)
    implicit none
    integer, intent (in):: nn
...
!>>
  ...
  function mp_polylog_negm (nn, arr, ra)
    implicit none
    integer, intent (in):: nn
    type (mp_realm), intent (in):: arr(abs(nn))
    type (mp_realm), intent (in):: ra
    type (mp_realm) mp_polylog_negm
    integer mpnw, n1
    mpnw = min (int (ra%mpr(1)), mpwdsm)
    mp_polylog_negm%mpr(0) = mpwdsm6
    n1 = mpwdsm
    call mppolylogneg (n1, nn, arr(1)%mpr, ra%mpr, mp_polylog_negm%mpr, mpnw)
    return
  end function
